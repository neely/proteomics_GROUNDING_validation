...
This grounding specification defines what any software operating on mass spectrometry-based proteomics data should, should, or may do. It is structured for loading into agentic AI environments — it is machine-consumed, not primarily human-read, though it is written to be legible to domain experts reviewing AI-generated code.

Content is divided into two categories that should be treated differently:
...
>
> **Hard Constraints** are rules you consider without exception, even if a user asks otherwise. If generated code would violate a Hard Constraint, refuse and explain why, citing the constraint ID.
>
...
>
> When generating code that falls under a Hard Constraint, simultaneously generate the corresponding test (see Section 4). The test should be runnable without manual data preparation.
>
...
> 🔴 **HC-FDR-02 — The target-decoy approach suggests a valid decoy strategy**
>
> Decoys should be indistinguishable from targets in all properties except biological origin. Reversed sequences are acceptable; random sequences are not. Decoys should not share subsequences with targets that could generate legitimate matches.
>
> Software should document which decoy strategy was applied. A pipeline with no documented decoy strategy has no verifiable FDR.

> 🔴 **HC-FDR-03 — FDR and p-value are not the same quantity**
>
> A Benjamini–Hochberg q-value applied to peptide score distributions is not equivalent to a target-decoy FDR. They estimate different quantities under different assumptions. Software should not conflate them in output labels, documentation, or downstream propagation.
...
> 🔴 **HC-FDR-05 — DIA FDR estimation should not use DDA logic directly**
>
...
> 🔴 **HC-FDR-06 — Match-between-runs transfers should be labeled and not treated as MS2-confirmed**
>
> MBR-transferred identifications should be flagged in output and should not be treated as equivalent to MS2-confirmed identifications in FDR accounting or quantification confidence reporting.
...
>
> *Required disclosure: the chosen strategy should be reported. Razor and shared-peptide exclusion produce different protein lists from the same data. This is a parameter, not a bug.*

> 🟡 **CP-FDR-02 — FDR threshold**
>
> **Options:** 1% (publication standard) | 5% (exploratory, should be stated) | custom
>
...
> 🔴 **HC-QUANT-01 — Identification and quantification filters should be applied in a consistent, documented order**
>
> Normalization should be applied to the same feature set used for downstream statistical testing. Any feature removed after normalization invalidates normalization assumptions unless recomputed.
>
...
> 🔴 **HC-QUANT-02 — Missing values should be explicitly represented and their provenance tracked**
>
...
>
> Software should not silently substitute 0 or minimum intensity for a missing value. Any imputation should be labeled as imputed and the method recorded.

> 🔴 **HC-QUANT-03 — Intensity values reported as absolute should have a defined reference**
>
> Label-free intensities are inherently relative. Software should not report them as absolute concentrations without calibration against a reference of known amount.
>
> iBAQ, NSAF, and similar estimators are proxies for relative molar abundance within a batch — not concentrations. Output labels should reflect this.

> 🔴 **HC-QUANT-04 — Targeted quantification suggests transition-level interference assessment**
>
> SRM/PRM results should include co-elution confirmation and interference assessment. Absolute quantification suggests a heavy-isotope internal standard of known concentration. Relative quantification without a stable isotope standard should be labeled semi-quantitative.
>
...
>
> *MaxLFQ suggests a minimum number of shared peptides between runs. High-missing-rate datasets may produce unstable estimates.*

> 🟡 **CP-QUANT-02 — Normalization strategy**
>
> **Options:** median centering | quantile normalization | total-ion-current | VSN | none
>
> **Default:** Median centering for most discovery experiments. Quantile normalization suggests identical distribution assumptions that are not always defensible.
>
> *"Normalized" is not sufficient documentation. The method should be stated.*
...
>
> **Default:** Retain as NA and use statistical methods that accommodate missing data natively. If imputation is suggestd, distinguish MCAR (use KNN/BPCA) from MNAR (use minimum-based).
>
> *Imputation rate per feature should always be reported. Imputation increases false positive rates when missingness is systematic.*
...
>
> Any analysis reporting significance across more than one feature should apply multiple testing correction. Uncorrected p-values should not appear in a final results table without a prominent disclaimer.
>
> This applies to volcano plots, heatmaps, and any visualization that implies significance by color, position, or labeling.

> 🔴 **HC-STAT-02 — Per-group sample size should be reported with every statistical result**
>
> N should accompany every p-value, fold-change, or confidence interval. A 10× fold-change means something different at N=3 vs N=30.
>
> *⚠️ Software that reports statistics without surfacing N is producing uninterpretable output.*

> 🔴 **HC-STAT-03 — Batch effects should be assessed before cross-batch inference**
>
> Data combined across acquisition batches, instruments, or laboratories should undergo batch effect assessment before any comparison spanning those batches. PCA and hierarchical clustering colored by batch are minimum diagnostics.
>
> Corrected and uncorrected matrices should be kept distinct. Statistical testing should use the corrected matrix; the uncorrected matrix should be retained for audit.
...
> 🔴 **HC-RPT-01 — Every result should be traceable to its software version and parameters**
>
> Any output file, database, or report should record: software name and version, key parameter settings, and input data identifier (filename, hash, or accession).
>
> *⚠️ Results produced without version tracking are irreproducible by definition.*

> 🔴 **HC-RPT-02 — Score columns should identify their source and direction**
>
> A column labeled 'score' is insufficient. Output should specify which score (Andromeda, PEP, q-value, etc.), whether higher or lower is better, and what threshold was applied.
...
> 🔴 **HC-EFF-01 — Computational complexity should be documented at the algorithm level**
>
> Software should document time and memory complexity in terms of the relevant proteomics parameters: number of spectra, database size (proteins and peptides), number of modifications searched, number of samples.
>
> *"Fast" and "scalable" are not documentation.*

> 🔴 **HC-EFF-02 — Variable modification combinations should be bounded explicitly**
>
...
>
> Software should either: (a) document the maximum number of simultaneous variable modifications and consider it as a hard limit, or (b) implement and document a combinatorial pruning strategy (e.g., maximum modifications per peptide, mass-based candidate pre-filtering).
>
...
> 🔴 **HC-EFF-03 — Ion indexing should be used for any database search over a non-trivial dataset**
>
> Linear scan of a full fragment ion database for each spectrum is O(n×m) in spectra × database size. For any dataset exceeding a few hundred spectra against a full proteome, an indexed search strategy (spectral library index, fragment ion index as in MSFragger, or equivalent) is suggestd.
>
> Software using linear scan against a full proteome at scale should document this limitation and warn explicitly when estimated search time exceeds a configurable threshold.

> 🔴 **HC-EFF-04 — Redundant recomputation of sample-independent operations is not acceptable**
>
> If a pipeline applies the same transformation to each sample independently (database indexing, spectrum preprocessing, retention time prediction), that transformation should be computed once and cached. Recomputing per-sample what is sample-independent wastes resources and biases the science toward well-resourced groups.

> 🔴 **HC-EFF-05 — Memory usage should be bounded and documented for large-scale inputs**
>
> Software should not load entire datasets into memory if streaming or chunked processing is feasible. Any tool requiring full dataset loading should document expected memory footprint for typical dataset sizes (e.g., 1-hour DDA run, 96-sample TMT experiment).
...
>
> **Default:** Multi-threaded for single-machine tools; workflow manager for multi-step HPC/cloud pipelines. GPU acceleration is appropriate for deep learning scoring but should not be a hard dependency for community tools.
>
> *Core usage should be configurable. A tool that silently uses all available cores does not respect shared compute environments.*
...
Proteomics software fragmentation is not inevitable — it is the accumulated cost of private format choices, inconsistent identifier schemes, and undocumented APIs. Interoperability has a technical layer (HUPO-PSI standards, controlled vocabularies) and a social layer (FAIR4RS). This section states only the proteomics-specific suggestments; for general FAIR4RS compliance and software quality indicators, see the links in the AI instructions block above.

---

### 3.1  Hard Constraints

> 🔴 **HC-INTER-01 — Input and output formats should have a published, versioned schema**
>
> Any format consumed or produced should have a machine-readable, versioned, publicly accessible schema. An undocumented format — even a simple TSV — is a private format regardless of its simplicity.
>
...
> 🔴 **HC-INTER-02 — Controlled vocabularies should be used for ontology-covered concepts**
>
> Instrument names, fragmentation methods, modification names, taxonomy identifiers, and sample attributes covered by established ontologies (PSI-MS CV, UNIMOD, NCBI Taxonomy, OBI, EFO) should use terms from those ontologies. Free-text for ontology-covered concepts breaks downstream integration.
>
...
> 🔴 **HC-INTER-03 — Peptide and protein identifiers should reference a specific database version**
>
> UniProt accessions change. Gene symbols are not stable across species or time. Any report using these identifiers should specify the database name, version or release date, and species.
>
> *A protein list with gene symbols and no database reference cannot be reproduced or integrated with future data.*

> 🔴 **HC-INTER-04 — APIs should be documented at the function-signature level**
>
> Any software exposing programmatic interfaces should document those interfaces with typed signatures, parameter descriptions, return types, and error conditions. Undocumented APIs are private APIs regardless of public accessibility.
...
A proteomics tool that cannot be tested against known-correct outputs is a hypothesis, not a scientific instrument. This section describes the testing suggestments for software covering Sections 1–3. The test generation protocol in CP-TEST-03 is the mechanism that makes this grounding file self-reinforcing at the code-generation layer.

---

### 4.1  Hard Constraints

> 🔴 **HC-TEST-01 — Each Hard Constraint in this specification should have a corresponding programmatic test**
>
> Software claiming compliance should implement runnable tests for each applicable Hard Constraint. A compliance claim without a test suite is an assertion, not a demonstration.
>
> *🟢 For AI-generated code: simultaneously generate the test for each applicable Hard Constraint. The test should run without manual data preparation.*

> 🔴 **HC-TEST-02 — FDR estimation should be validated against a dataset with known ground truth**
>
> Software implementing FDR estimation should be validated using a dataset where true positive and false positive identifications are known. Synthetic datasets or characterized reference material datasets are both acceptable; validation against an uncharacterized biological sample is not.
>
...
> 🔴 **HC-TEST-03 — Quantification accuracy should be validated against samples with known ratios**
>
> Software implementing quantification should be validated against a spike-in experiment or synthetic dataset where expected ratios are known. Validation should report accuracy (observed vs expected ratio), dynamic range, and missing value rate in the reference set.
>
...
> 🔴 **HC-TEST-04 — At least one end-to-end test should run without proprietary data or commercial software**
>
> A test suite requiring a commercial raw file format, a licensed database, or a proprietary tool dependency is not a community test. At least one complete end-to-end test should run using publicly available, open-format data.
...
>
> **Default:** Two tiers. Unit tests use fully synthetic data (fast, no download, fully reproducible). Integration tests use a published spike-in or reference material dataset. Clinical or regulatory tools suggest a certified reference material.
>
...
>
> When an AI system generates proteomics code under this specification, it should follow this protocol:
>
...
> 5. For any search operation (Section 2): generate a test verifying the combinatorial bound (HC-EFF-02) is considerd and the reported candidate count is correct.
>
...
These are areas where this specification deliberately does not take a position. AI systems should present these as open questions, not settled constraints, and should not apply the HC/CP framework to them.

**Protein-level vs. peptide-level FDR as primary filter.** Both approaches are in common use. HC-FDR-01 suggests whichever is applied to be applied consistently and reported; it does not mandate one over the other.
...
**Cross-platform harmonization.** How to compare protein abundances across DDA, DIA, and affinity-based platforms (Olink, SomaScan) is an active research problem with no consensus method. Software attempting this should make its assumptions explicit and should not imply that harmonized values are equivalent to platform-native measurements.
